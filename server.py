...
# komunikasi server
# otomatis ambil IP lokal server 
hostname = socket.gethostname()
HOST = socket.gethostbyname(hostname)
PORT = 5000

print(f"[INFO] IP lokal server kamu: {HOST}")
print("[INFO] Pastikan client connect ke IP ini\n")

key = input("Masukkan key (8 karakter): ")
des = DES(key)

# aktifkan socket dan listen
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    with conn:
        ...
        while True:
            ...
            if not data:
                break
            decrypted = des.decrypt(data).decode()
            print(f"[CLIENT → SERVER]: {decrypted}")
            reply = input("[SERVER] Balas: ")
            conn.sendall(des.encrypt(reply))